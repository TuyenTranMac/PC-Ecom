...
async function main() {
  ...
  // Mapping category slug → product images
  const imagesByCategory: Record<string, string[]> = {
    ...
    // CPU
    "cpu-intel": ["/cpu/cpu (1).jpg", "/cpu/cpu (2).jpg", "/cpu/cpu (3).jpg"],
    "cpu-amd": ["/cpu/cpu (4).jpg", "/cpu/cpu (5).jpg", "/cpu/cpu (6).jpg"],
    ...
    // RAM
    "ram-corsair": ["/ram/ram (1).jpg", "/ram/ram (2).jpg", "/ram/ram (3).jpg"],
    "ram-gskill": ["/ram/ram (2).jpg", "/ram/ram (3).jpg", "/ram/ram (4).jpg"],
    "ram-kingston": [
      "/ram/ram (3).jpg",
      "/ram/ram (4).jpg",
      "/ram/ram (5).jpg",
    ],
    "ram-adata": ["/ram/ram (4).jpg", "/ram/ram (5).jpg", "/ram/ram (6).jpg"],
    "ram-teamgroup": [
      "/ram/ram (5).jpg",
      "/ram/ram (6).jpg",
      "/ram/ram (1).jpg",
    ...
    // SSD
    "ssd-samsung": ["/ssd/ssd (1).jpg", "/ssd/ssd (2).jpg", "/ssd/ssd (3).jpg"],
    "ssd-wd": ["/ssd/ssd (2).jpg", "/ssd/ssd (3).jpg", "/ssd/ssd (4).jpg"],
    "ssd-kingston": [
      "/ssd/ssd (3).jpg",
      "/ssd/ssd (4).jpg",
      "/ssd/ssd (5).jpg",
    ],
    "ssd-crucial": ["/ssd/ssd (4).jpg", "/ssd/ssd (5).jpg", "/ssd/ssd (1).jpg"],
  };
  ...
  // Helper function để lấy images cho category, fallback nếu không có
  const getImagesForCategory = (categorySlug: string): string[] => {
    const images = imagesByCategory[categorySlug];
    if (images) {
      return shuffleArray(images); // Xáo trộn thứ tự
    }
  ...
  for (const productData of products) {
    ...
    // Sample images - dùng helper function
    const images = getImagesForCategory(category.slug);
  ...
  // ==========================================
  // SEED 200 PRODUCTS CHO SHOP "conmeocute"
  // ==========================================
  console.log("🎯 Bắt đầu seed 200 sản phẩm cho shop conmeocute...");

  const conmeocuteStore = await prisma.store.findFirst({
    where: {
      User: {
        username: "conmeocute",
      },
  ...
  if (!conmeocuteStore) {
    console.log("⚠️ Không tìm thấy shop với username conmeocute");
  } else {
    console.log(
      `✓ Tìm thấy shop: ${conmeocuteStore.name} (${conmeocuteStore.slug})`
    );

    // Các category slugs để random
    const categorySlugs = [
      "laptop-asus",
      "laptop-msi",
      "laptop-acer",
    ...
    for (let i = 1; i <= 200; i++) {
      ...
      if (!category) {
        console.log(
          `⚠️ Không tìm thấy category ${randomCategorySlug}, bỏ qua sản phẩm ${i}`
        );
        continue;
      }

      // Generate tên sản phẩm random
      const prefix =
      ...
      const productName = `${prefix} ${type} ${category.name} ${modelNumber}`;
      const productSlug = `conmeo-${i}-${modelNumber}`;

      // Random price (1tr - 50tr)
      const price = Math.floor(Math.random() * 49000000) + 1000000;
      const comparePrice = Math.floor(
      ...
      const descriptions = [
        "Sản phẩm chất lượng cao, hiệu năng vượt trội",
        "Thiết kế hiện đại, công nghệ tiên tiến",
        "Giá tốt, uy tín, bảo hành chính hãng",
        "Hot trend 2024, đánh giá 5 sao",
        "Phù hợp mọi nhu cầu, độ bền cao",
        "Tối ưu cho gaming, làm việc, sáng tạo",
        "Build quality tốt, giá trị đồng tiền",
        "Khuyên dùng cho game thủ chuyên nghiệp",
      ];
      const description =
      ...
      await prisma.product.create({
        data: {
          ...
          isArchived: false,
          categoryId: category.id,
          storeId: conmeocuteStore.id,
        },
      });

      if (i % 20 === 0) {
        console.log(`  ✓ Đã tạo ${i}/200 sản phẩm`);
      }
    }

    console.log(`✅ Đã tạo 200 sản phẩm cho shop ${conmeocuteStore.name}!`);
  }
